  ...
  return (
    <div id="skills" className="w-full bg-[#0a192f] text-gray-300">
      <div className="max-w-[1000px] mx-auto p-4 flex flex-col justify-center w-full h-full">
        ...
        {/* <div className="uppercase font-bold border-b-2 border-pink-600">
          language
        </div>
        <div className="w-full grid grid-cols-3 sm:grid-cols-6 gap-4 text-center py-8">
          {skills.language.map(({ name, level }, index: any) => {
            return (
              <div
                key={`skill_${index}`}
                className="shadow-md shadow-[#040c16] hover:scale-110 duration-500 border-2 border-gray-800 rounded-lg"
              >
                <div className="w-full bg-gray-200 rounded-full dark:bg-gray-700">
                  <div
                    className="bg-green-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full"
                    style={{ width: `${Number(level) * 10}%` }}
                  >
                    {`${Number(level) * 10}%`}
                  </div>
                </div>
